...
const ManageProductsPage = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      let url = `${API_BASE_URL}/my-barang`;
      if (filterStatus !== "all") {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/barang/${editingProduct.item_id}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
                ...
                <div className="flex items-center">
                  <div className="h-10 w-10 flex-shrink-0 mr-3 bg-gray-200 rounded-md overflow-hidden">
                    {product.image_url || product.image_data ? (
                      <img
                        className="h-10 w-10 object-cover transition-opacity duration-300 rounded"
                        src={`/api/barang/${product.item_id}/image`}
                        alt={product.item_name}
                        onLoad={(e) => e.target.classList.remove('opacity-0')}
                        onError={(e) => {
                          e.target.style.display = 'none';
                          const placeholder = document.createElement('div');
                          placeholder.className = 'h-10 w-10 bg-gray-200 rounded flex items-center justify-center';
                          placeholder.innerHTML = '<svg class="w-5 h-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>';
              ...
              <div className="mb-6">
                ...
                <div className="mt-1 flex items-center">
                  {newProduct.imagePreview ? (
                    ...
                    <label className="w-32 h-32 flex items-center justify-center border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-green-500">
                      <input
                        type="file"
                        className="hidden"
                        accept="image/*"
