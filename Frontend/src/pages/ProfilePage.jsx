...
const ProfilePage = () => {
  ...
  const fetchTransactions = async (type) => {
    ...
    const response = await fetch(`${API_BASE_URL}/transaksi?type=${type}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
  ...
  const getProfileImageUrl = (imagePath) => {
    if (!imagePath) {
      // Return default avatar SVG instead of local asset
      return null; // Will show default icon
    }
    // If it's a relative path like '/uploads/profiles/profile-image.png'
  ...
  const handleUpdateStatus = async (transactionId, newStatus) => {
    try {
      ...
      await fetchTransactions(activeTab === 'purchases' ? 'purchase' : 'sale');
              ...
              <div className="bg-white rounded-lg shadow-md overflow-hidden">
                ...
                <div className="p-6 text-center border-b">
                  <div className="relative inline-block mb-3">
                    ...
                    <div 
                      className={`w-24 h-24 rounded-full mx-auto bg-gradient-to-br from-green-400 to-green-600 border-4 border-green-100 flex items-center justify-center ${user?.profile_picture ? 'hidden' : 'flex'}`}
                    >
                        ...
                        {transactions.map(transaction => (
                          <div key={transaction.transaction_id || transaction.id} className="border rounded-lg p-4 hover:bg-gray-50 transition">
                            ...
                            <div className="flex items-center">
                              ...
                              <div className={`w-16 h-16 bg-gradient-to-br from-gray-200 to-gray-300 rounded-md mr-4 flex items-center justify-center ${transaction.item?.image_url ? 'hidden' : 'flex'}`}>
                              ...
                              <div className="text-right">
                                ...
                                <div className="flex items-center justify-end mt-1">
                                  <button
                                    onClick={() => navigate(`/transaksi/${transaction.transaction_id || transaction.id}`)}
                        ...
                        {sales.map(sale => (
                          <div key={sale.transaction_id || sale.id} className="border rounded-lg p-4 hover:bg-gray-50 transition">
                            ...
                            <div className="flex items-center">
                              ...
                              <div className={`w-16 h-16 bg-gradient-to-br from-gray-200 to-gray-300 rounded-md mr-4 flex items-center justify-center ${sale.item?.image_url ? 'hidden' : 'flex'}`}>
                                <svg className="w-6 h-6 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                </svg>
                              </div>
                              ...
                              <div className="text-right">
                                ...
                                <div className="flex items-center justify-end mt-1">
                                  <button
                                    onClick={() => navigate(`/transaksi/${sale.transaction_id || sale.id}`)}
                                    className="text-sm text-blue-600 hover:text-blue-800 flex items-center mr-3"
                                  >
                                    <span>Lihat Detail</span>
                                    <HiEye className="ml-1" />
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center space-x-4 py-4">
                <img 
                  src={getProfileImageUrl(selectedTransaction.item?.image_url)} 
                  alt={selectedTransaction.item?.item_name}
                  className="w-24 h-24 object-cover rounded-lg"
                />
                <div className="flex-1">
                  <h4 className="font-medium">{selectedTransaction.item?.item_name}</h4>
                  <p className="text-gray-600">Jumlah: {selectedTransaction.quantity}</p>
                  <p className="text-green-600 font-semibold">
                    Rp {selectedTransaction.total_price.toLocaleString('id-ID')}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 py-4 border-t border-b">
                <div>
                  <p className="text-sm text-gray-600">
                    {activeTab === 'purchases' ? 'Penjual' : 'Pembeli'}
                  </p>
              ...
              {/* Action Buttons - Only show for seller */}
              {activeTab === 'sales' && selectedTransaction.status === 'pending' && (
                <div className="flex justify-end space-x-3 pt-4">
                  <button
                    onClick={() => handleUpdateStatus(selectedTransaction.transaction_id, 'cancelled')}
                  ...
                  <button
                    onClick={() => handleUpdateStatus(selectedTransaction.transaction_id, 'completed')}
                    className="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700"
                    disabled={isUpdatingStatus}
                  >
                    {isUpdatingStatus ? 'Memproses...' : 'Konfirmasi Selesai'}
                  </button>
                </div>
              )}
            </div>
